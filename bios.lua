do
    local _call_stack = {}
    local native_getinfo = debug.getinfo
    local native_traceback = debug.traceback
    local native_error = error
    local native_resume = coroutine.resume
    local native_status = coroutine.status
    local native_create = coroutine.create
    local native_getfenv = getfenv
    local native_setfenv = setfenv
    local coroutineList = {}
    local fullCoroutineList = {[0] = coroutine.running()}
    local old_hook = debug.gethook()

    local function hook(ev)
        if ev == "call" then table.insert(_call_stack, native_getinfo(2))
        elseif ev == "return" or ev == "tail return" then table.remove(_call_stack, #_call_stack) 
        elseif ev == "get stack" then return _call_stack end
    end

    debug.sethook(hook, "cr")

    local function getThreadStack(th, idx)
        --print("getThreadStack(" .. th .. ", " .. idx .. ")")
        if th < 0 then return nil end
        if fullCoroutineList[th] == nil then return nil end
        local call_stack = debug.gethook(fullCoroutineList[th])("get stack")
        if call_stack == nil then return nil
        elseif #call_stack <= idx then 
            return getThreadStack(th - 1, idx - #call_stack)
        else 
            return native_getinfo(fullCoroutineList[th], idx) 
        end
    end

    local function stack(level)
        local i = level or 1
        return function()
            i=i+1
            return getThreadStack(#fullCoroutineList, i)
        end
    end
    ...
    function debug.getinfo(thread, func, what)
        if type(thread) ~= "thread" then
            what = func
            func = thread
            if type(func) ~= "number" or func == 0 then return native_getinfo(func, what) else
                --if func > #call_stack then return nil end
                local retval = getThreadStack(#fullCoroutineList, func)
                -- use what
                return retval
            end
        else return native_getinfo(thread, func, what) end
    end
    ...
    function debug.traceback(thread, message, level)
        if type(thread) ~= "thread" then
            level = message or 1
            message = thread
            --if level > #call_stack then return nil end
            local retval = message and message .. "\nstack traceback:" or "stack traceback:"
            for inf in stack(level) do
                if not (inf.what ~= "Lua" or inf.name == "xpcall" or inf.short_src == "(tail call)") then
                    retval = retval .. string.format("\n    %s:%d: in function <%s:%d>", inf.short_src or "?", inf.currentline or "?", inf.name or "?", inf.linedefined or "?")
                end
            end
            return retval
        else return native_traceback(thread, func, what) end
    end
    ...
    function _G.error(message, level)
        ...
        native_error(string.format("%s:%d: %s", info.short_src, info.currentline, message), -1)
    end

    function _G.getfenv(f)
        if type(f) == "number" then
            if f == 0 then return _G end
            local stack = getThreadStack(#fullCoroutineList, f)
            if stack == nil or stack.func == nil then return _G 
            else return native_getfenv(stack.func) end
        else return native_getfenv(f) end
    end

    function _G.setfenv(f, tab)
        if type(f) == "number" then
            if f == 0 then native_setfenv(0, tab) end
            local stack = getThreadStack(#fullCoroutineList, f)
            if stack == nil or stack.func == nil then return
            else return native_setfenv(stack.func, tab) end
        else return native_setfenv(f, tab) end
    end

    function _G.xpcall( _fn, _fnErrorHandler )
        local typeT = type( _fn )
        assert( typeT == "function", "bad argument #1 to xpcall (function expected, got "..typeT..")" )
        local co = native_create( _fn )
        local call_stack = {}
        local function hook(ev)
            if ev == "call" then table.insert(call_stack, native_getinfo(2))
            elseif ev == "return" or ev == "tail return" then table.remove(call_stack, #call_stack) 
            elseif ev == "get stack" then return call_stack end
        end
        debug.sethook(co, hook, "cr")
        table.insert(fullCoroutineList, co)
        local tResults = { native_resume( co ) }
        table.remove(fullCoroutineList, #fullCoroutineList)
        while native_status( co ) ~= "dead" do
            local ev = { coroutine.yield( unpack( tResults, 2 ) ) }
            table.insert(fullCoroutineList, co)
            tResults = { native_resume( co, table.unpack( ev ) ) }
            table.remove(fullCoroutineList, #fullCoroutineList)
        end
    ...
    function _G.pcall( _fnBase, ... )
        local typeT = type( _fnBase )
        assert( typeT == "function", "bad argument #1 to pcall (function expected, got "..typeT..")" )
        local tArgs = { ... }
        local _fn = function() return _fnBase( unpack ( tArgs ) ) end
        local _fnErrorHandler = function( _error ) return _error end
        local co = native_create( _fn )
        local call_stack = {}
        local function hook(ev)
            if ev == "call" then table.insert(call_stack, native_getinfo(2))
    ...
    function coroutine.create(func)
        ...
        debug.sethook(coro, hook, "cr")
        return coro
    end

    function coroutine.resume(coro, ...)
        table.insert(coroutineList, coro)
        table.insert(fullCoroutineList, coro)
        local res = {native_resume(coro, ...)}
    ...
    function coroutine.status(coro)
