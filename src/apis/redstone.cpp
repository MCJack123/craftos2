...
static int rs_setAnalogOutput(lua_State *L) {
    ...
    if (isnan(strength)) return luaL_error(L, "bad argument #2 (number expected, got nan)");
    if (isinf(strength)) return luaL_error(L, "bad argument #2 (number expected, got inf)");
...
static int rs_testBundledInput(lua_State *L) {
    ...
    if (isnan(mask)) return luaL_error(L, "bad argument #2 (number expected, got nan)");
    if (isinf(mask)) return luaL_error(L, "bad argument #2 (number expected, got inf)");
