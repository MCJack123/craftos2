...
int monitor::drawPixels(lua_State *L) {
    ...
    if (isSolidFill) {
        ...
        for (int h = max(-init_y, 0); h < cool_height; h++) {
            memset(&(*term->currentPixels())[init_y + h][memset_x], index, memset_len);
        }
    ...
    for (unsigned h = 0; h < height; h++) {
        ...
        if (lua_isstring(L, -1)) {
            ...
            if (str_offset < len)
                memcpy(&(*term->currentPixels())[init_y + h][init_x + str_offset],
                       str + str_offset,
                       len - str_offset
                );
            ...
            for (unsigned w = 0; w < max; w++) {
                ...
                if (lua_isnumber(L, -1)) {
                    ...
                    if (color >= 0)
                        (*term->currentPixels())[init_y + h][init_x + w] = term->mode == 2
                            ? lua_tointeger(L, -1)
